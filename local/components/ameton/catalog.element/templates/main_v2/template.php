...
<div class="block block-product _new" id="product">
	<div class="block__inner">

		<a href="javascript:void(0);" class="i-add-to-favorite i-mobile"
           <?if(count($arResult['OFFERS']) > 0){?>data-product-id="offer"<?}else{?> data-product-id="<?=$arResult['ID']?>"<?}?>
           data-price="<?=$price['RATIO_PRICE']?>"  data-discount="<?=$price['RATIO_PRICE']?>"
           data-action="add" data-text-success="Товар добавлен в избранное.">&nbsp;</a>

		<div class="block-product__images-videos">

			<div class="block-product__images slides" data-slick-slidestoshow="1" data-slick-slidestoscroll="1"
				...
				<div class="block-product__images-page slides__item slides gallery _desktop" data-slick-slidestoshow="1"
					...
					?>
						<figure class="block-product__figure slides__item <? if ($imgCount >= 6) { ?> _hide<? } ?>" itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
							<a href="<?=$image?>" class="block-product__link" itemprop="contentUrl" data-size="1900x2418">
	                            <img alt="<?=$arResult['NAME']?>" class="block-product__image i-lazy1" src="<?=$image?>" itemprop="thumbnail"></a>
						</figure>
					<? $imgCount++; } ?>
					...
					?>
						<div class="nob20-detail-video _desktop">
							<video class="nob20-detail-video__player" src="<?=$fileRes;?>" controls="true" preload="auto" autoplay="true" loop="true" muted="muted" playsinline></video>
						</div>
					<? } ?>

				</div>

				<div class="block-product__images-page slides__item slides gallery _mobile" data-slick-slidestoshow="1" 
					data-slick-slidestoscroll="1" data-slick-infinite="true" data-slick-variable-width="false" data-slick-touch-threshold="20" 
					data-slick-fade="true" data-slick-adaptive-height="true" data-slick-disable-on-pc="true" data-slick-menu=".block-product__buttons" 
			...
			<div class="block-product__buttons slides-menu i-noselect i-mobile" data-slick=".block-product__images-page">
				<?
					...
					foreach($arResult['SLIDER_PHOTOS'] as $image) {
				?>
				<div class="slides-menu__item <? if ($imgCount == 1) { ?>slides-menu__item_active<? } ?>" data-slick=".block-product__images-page"></div>
				<? $imgCount++; } ?>
			</div>
			...
			<?
				...
				$fileRes = CFile::GetPath($fileID);
			?>
				<div class="nob20-detail-video _mobile">
					<video class="nob20-detail-video__player" src="<?=$fileRes;?>" controls="true" preload="auto" autoplay="true" loop="true" muted="muted" playsinline></video>
				</div>
			<? } ?>
			...
			<script>/*
				var videos = document.querySelectorAll('.nob20-detail-video__player');
		...
		<div class="block-product__main">
			...
			<?if($arResult['PROPERTIES']['BREND']['VALUE']):?>
				<h1 class="block-product__title"><?=$arResult['PROPERTIES']['BREND']['VALUE']?></h1>
			<?endif;?>

			<?if($arResult['NAME']):?>
				<? if ($arResult['PROPERTIES']['ANGLIYSKOE_NAIMENOVANIE']['VALUE'] && LANGUAGE_ID == "en") { ?>
					<h2 class="block-product__subtitle"><?=$arResult['PROPERTIES']['ANGLIYSKOE_NAIMENOVANIE']['VALUE']?></h2>
				<? } else { ?>
					<h2 class="block-product__subtitle"><?=$arResult['NAME']?></h2>
				<? } ?>
			<?endif;?>
											...
											<div class="product-item-scu-container">
												<div class="product-item-scu-block">
													<div class="product-item-scu-list">
														...
														<?if ($skuProperty['SHOW_MODE'] === 'PICT' || $skuProperty['CODE'] =='TSVETT'):?>
                                                            <?if($arResult['PROPERTIES']['PREDZAKAZ']['VALUE']){
                                                                //Февраль 2020
                                                                $date_admission = "";
															...
															<ul class="block-product__colors block-product-colors slides-menu i-noselect" data-slick=".block-product__images">
																...
																<?foreach ($skuProperty['VALUES'] as &$value):
                                                                    ...
                                                                    foreach ($arResult['OFFERS'] as $ofs){
                                                                        if($ofs['PROPERTIES']['TSVETT']['VALUE'] == $value['NAME']) $offer_id_size = $ofs['ID'];
                                                                    }
                                                                    if($offer_id_size){
                    ...
                    <?}else{
                        if(count($arResult['OFFERS']) > 0){
                            ...
                            <div class="block-product__buttons-only" id="<?=$itemIds['BASKET_ACTIONS_ID']?>">
                                ...
                                {
                                    ?>
                                    <a href="javascript:void(0);" class="button button_important i-add-to-cart<?if (LANGUAGE_ID == 'en'){echo '-en';}?> <?=$showButtonClassName?>"
                                       title="<?=GetMessage('CT_BCE_CATALOG_ADD')?>" data-text-success="<?=GetMessage('PROD_IN_BASKET')?>"
                                       data-product-id="offer" data-price="<?=$price['RATIO_BASE_PRICE']?>" data-discount="<?=$price['RATIO_PRICE']?>" data-action="add_basket"
                                ...
                                if ($showBuyBtn)
                                {
                                    ...
                                    <a href="javascript:void(0);" class="button button_important i-add-to-cart<?if (LANGUAGE_ID == 'en'){echo '-en';}?> <?=$showButtonClassName?>"
                                       title="<?=GetMessage('CT_BCE_CATALOG_ADD')?>" data-text-success="<?=GetMessage('PROD_IN_BASKET')?>"
                                       data-product-id="offer" data-price="<?=$price['RATIO_BASE_PRICE']?>" data-discount="<?=$price['RATIO_PRICE']?>"
                                ...
                                <a href="<?if (LANGUAGE_ID == 'ru'){echo '#form-order';}else{echo '#form-order-en';}?>" class="button i-modal" id="fast-order"
                                   data-ib="<?if (LANGUAGE_ID == 'ru'){echo '4';}else{echo '9';}?>" data-prod="offer"
                                   title="<?=GetMessage('FAST_BUY')?>"><?=GetMessage('FAST_BUY')?></a>
                            </div>
                            <a href="javascript:void(0);" id="add-to-favorite" class="i-add-to-favorite"
                               data-product-id="offer" data-price="<?=$price['RATIO_BASE_PRICE']?>" data-discount="<?=$price['RATIO_PRICE']?>"
                               data-text-success="<?=GetMessage('IN_FAV')?>">&nbsp;</a>
                            <?
                        }else{
                            ?>
                            <div class="block-product__buttons-only" id="<?=$itemIds['BASKET_ACTIONS_ID']?>">
                                ...
                                if ($showAddBtn)
                                {
                                    ?>
                                    <a href="javascript:void(0);" class="button button_important i-add-to-cart<?if (LANGUAGE_ID == 'en'){echo '-en';}?> <?=$showButtonClassName?>"
                                       title="В корзину" data-product-id="<?=$arResult['ID']?>" data-text-success="Товар добавлен в вашу корзину."
                                ...
                                <a href="#form-order" class="button i-modal" id="fast-order"
                            ...
                            <a href="javascript:void(0);" id="add-to-favorite" class="i-add-to-favorite"
                               ...
                               data-discount="<?=$price['RATIO_PRICE']?>"
                               data-text-success="Товар добавлен в избранное.">&nbsp;</a>
                            <?
                        }
                        ?>
...
if (LANGUAGE_ID == 'en'){echo "You might also like";}else{echo html_entity_decode($res['NAME']);}
					...
					$ids = [];
                    //echo "<pre>"; print_r($arResult['PROPERTIES']['REKOMENDUEMYY_TOVAR']['VALUE']); echo "</pre>";
					if($arResult['PROPERTIES']['REKOMENDUEMYY_TOVAR']['VALUE']){
                       $arts = $arResult['PROPERTIES']['REKOMENDUEMYY_TOVAR']['VALUE'];
                       ...
                       //echo "<pre>"; print_r($arts); echo "</pre>";
                        ...
                        foreach ($arts as $art){
                            $r = CIBlockElement::GetList(
                                array('ID' => 'DESC'),
                                array('IBLOCK_ID' => 1, 'ACTIVE' => 'Y', 'CHECK_PERMISSIONS' => 'Y', 'PROPERTY_CML2_ARTICLE' => $art),
                                false,
                                array('nTopCount' => 1),
                                array('ID')
                            );
                        ...
                        $r = CIBlockElement::GetList(
                            array('RAND' => 'rand'),
                            array('IBLOCK_ID' => 1, 'ACTIVE' => 'Y', 'CHECK_PERMISSIONS' => 'Y', 'ACTIVE_DATE' => 'Y'),
                            false,
                            array('nTopCount' => 3),
                            array('ID')
                        );
                        ...
                        $r = CIBlockElement::GetList(
                            array('RAND' => 'rand'),
                            array('IBLOCK_ID' => 8, 'ACTIVE' => 'Y', 'CHECK_PERMISSIONS' => 'Y', 'ACTIVE_DATE' => 'Y'),
                            false,
                            array('nTopCount' => 3),
                            array('ID')
                        );
...
<div style="display: none;" class="bx-catalog-element bx-<?=$arParams['TEMPLATE_THEME']?>" id="<?=$itemIds['ID']?>"
