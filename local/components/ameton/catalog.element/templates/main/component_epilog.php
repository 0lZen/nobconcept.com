...
if (LANGUAGE_ID == "en")
{
    if ($arResult['NAME_EN'])
    {
        $name=$arResult['NAME_EN'];
    }
	$APPLICATION->SetPageProperty("title", $name.' – купить в интернет-магазине NOBconcept');
	$APPLICATION->SetPageProperty("description", $name.' в интернет-магазине NOBconcept. ✈ Доставка по всей России. ☎ +7 (495) 784-04-02');
} else
{
    $APPLICATION->SetPageProperty("title", $name.' – купить в интернет-магазине NOBconcept');
    $APPLICATION->SetPageProperty("description", $name.' в интернет-магазине NOBconcept. ✈ Доставка по всей России. ☎ +7 (495) 784-04-02');
...
foreach ($arResult['BREADCRUMBS'] as $bc)
{
    $APPLICATION->AddChainItem($bc['NAME'], $bc['URL']);
}
