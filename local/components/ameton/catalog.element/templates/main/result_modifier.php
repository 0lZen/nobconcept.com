...
foreach ($arResult['SECTION']['PATH'] as $sect)
{
    ...
    } else
    {
        ...
        $db_list = CIBlockSection::GetList(Array($by=>$order), $arFilter, false,['UF_NAME_EN']);
        while($ar_result = $db_list->Fetch())
        {
            if ($ar_result['UF_NAME_EN'])
            {
                $arResult['BREADCRUMBS'][]=[
                    'NAME' => $ar_result['UF_NAME_EN'],
                    'URL'  => $sect['SECTION_PAGE_URL'],
                ];
            } else
            {
...
$arResult['NAME_EN']=$arResult['PROPERTIES']['ANGLIYSKOE_NAIMENOVANIE']['VALUE'];

$component = $this->getComponent();
$component->SetResultCacheKeys(array('BREADCRUMBS','NAME','NAME_EN'));
