...
}try{j.prop({autoplay:l.autoplay,loop:l.loop,volume:l.volume,muted:l.muted,playsinline:true,defaultMuted:l.muted,playbackRate:l.playbackRate,defaultPlaybackRate:l.playbackRate});
g(j).attr("playsinline",true);}catch(o){throw new Error(a);}j.css({margin:"auto",position:"absolute","z-index":-1,top:m.y,left:m.x,"-webkit-transform":"translate(-"+m.x+", -"+m.y+")","-ms-transform":"translate(-"+m.x+", -"+m.y+")","-moz-transform":"translate(-"+m.x+", -"+m.y+")",transform:"translate(-"+m.x+", -"+m.y+")",visibility:"hidden",opacity:0}).one("canplaythrough."+e,function(){q.resize();
