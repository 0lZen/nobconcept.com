...
?>
<!DOCTYPE html>
<html lang="<?=LANGUAGE_ID?>" prefix="og: http://ogp.me/ns# fb: http://ogp.me/ns/fb#" class="l-site">
<head>
    <meta charset="UTF-8">
    <?
    ...
    {
        ?>
        <link rel="alternate" hreflang="ru" href="https://<?=$_SERVER['SERVER_NAME']?><?=$_SERVER['REQUEST_URI']?>" />
        <link rel="alternate" hreflang="en-us" href="https://<?=$_SERVER['SERVER_NAME']?>/en<?=$_SERVER['REQUEST_URI']?>" />
        <?
    } else
    if (LANGUAGE_ID=='en')
    {
        ?>
        <link rel="alternate" hreflang="ru" href="https://<?=$_SERVER['SERVER_NAME']?><?=substr($_SERVER['REQUEST_URI'],3)?>" />
        <link rel="alternate" hreflang="en-us" href="https://<?=$_SERVER['SERVER_NAME']?><?=$_SERVER['REQUEST_URI']?>" />
        <?
    }
    if ($_SERVER['HTTP_USER_AGENT'] === "TelegramBot (like TwitterBot)" || stristr($_SERVER['HTTP_USER_AGENT'], 'WhatsApp'))
    {
        ?>
    ...
    <?} elseif ($APPLICATION->GetCurDir() === (LANGUAGE_ID === 'ru' ? '/' : '/' . LANGUAGE_ID . '/') . 'ui/' || mb_strpos($APPLICATION->GetCurDir(), (LANGUAGE_ID === 'ru' ? '/' : '/' . LANGUAGE_ID . '/') . 'catalog/',0,'UTF-8') === 0) {?>
      ...
      <?}?>
    <?} elseif (
    mb_strpos($APPLICATION->GetCurDir(), (LANGUAGE_ID === 'ru' ? '/' : '/' . LANGUAGE_ID . '/') . 'shop/',0,'UTF-8') === 0
    || mb_strpos($APPLICATION->GetCurDir(), (LANGUAGE_ID === 'ru' ? '/' : '/' . LANGUAGE_ID . '/') . 'brands/',0,'UTF-8') === 0) {?>
    ...
    <meta name="facebook-domain-verification" content="8s1a5xfcd1tzdejn96oo20m95bkvdk" />
...
<body itemscope itemtype="http://schema.org/WebSite" class="l-page <?=(ERROR_404 === "Y" ? "l-page_common l-page_screen" : "")?>">
    ...
    <div class="wrapper">
		<nav class="block block-navigation<?=($APPLICATION->GetCurDir() === SITE_DIR && ERROR_404 !== "Y" ? ' block-navigation_index' : '')?>" id="navigation_header">
        ...
        <?$APPLICATION->ShowViewContent('subnavigation');?>
        ...
        <nav class="block block-menu i-mobile" id="menu">
            ...
            <?$APPLICATION->ShowViewContent('brands_mobile_subnavigation');?>
		    <main>

            <?
            if (
                $APPLICATION->GetCurDir() !== SITE_DIR
                && mb_strpos($APPLICATION->GetCurDir(), SITE_DIR.'catalog/')!==0
                && mb_strpos($APPLICATION->GetCurDir(), SITE_DIR.'sale/')!==0
                && mb_strpos($APPLICATION->GetCurDir(), SITE_DIR.'pre-order/')!==0
                && ERROR_404 !== "Y"
            )
            {
                ?>
