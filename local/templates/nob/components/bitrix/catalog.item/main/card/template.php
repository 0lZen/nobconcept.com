...
{
    $keys=['UNROUND_BASE_PRICE','UNROUND_PRICE','BASE_PRICE','PRICE','DISCOUNT','RATIO_BASE_PRICE','RATIO_PRICE','RATIO_DISCOUNT'];

    foreach ($keys as $keyPrice)
    {
        $price[$keyPrice]=round(CCurrencyRates::ConvertCurrency($price[$keyPrice], "RUB", $arParams['CURRENCY']),2);
        $price['PRINT_'.$keyPrice]=CurrencyFormat($price[$keyPrice], $arParams['CURRENCY']);
    }
}

$mainImg=smart_resize_ameton($item['PREVIEW_PICTURE']['ID'],$w,$h,JPEG_QUALITY,false,false,true);
#$mainImg=$item['PREVIEW_PICTURE']['SRC'];
?>
<div class="product" id="<?=$areaId?>" data-entity="items-row">
	<div class="product__images">
		<div class="product__image product__image_main i-lazy" data-bg="url('<?=$mainImg?>')" ></div>
        <?
        if($item['PROPERTIES']['MORE_PHOTO']['VALUE'] && !$arParams["IS_PHONE"])
        {
            #$extra_img = CFile::GetPath($item['PROPERTIES']['MORE_PHOTO']['VALUE'][0]);
            $extra_img=smart_resize_ameton($item['PROPERTIES']['MORE_PHOTO']['VALUE'][0],$w,$h,JPEG_QUALITY,false,false,true);
            ?>
            <div class="product__image product__image_second i-lazy" data-bg="url('<?=$extra_img?>')" ></div>
            <?
        }
        ?>
		...
		{
			?>
			<div class="product__image product__image_second i-lazy" data-bg="url('<?=$photo['SRC']?>')" data-was-processed="true" style="background-image: url('<?=$photo['SRC']?>');"></div>
			<?
		}*/?>
	</div>
	<div class="product__content">
		<h3 class="product__brand"><?=$brand?></h3>
		<h4 class="product__title"><?=$productTitle?></h4>
		<div class="product__price-wrapper <?if (!empty($price)&& $price['RATIO_DISCOUNT'] > 0){?>has-old<?}
		if($item['PROPERTIES']['PREDZAKAZ']['VALUE']){?> has-preorder<?}?><?
		if($price['RATIO_PRICE'] <= 0){?> has-sold<?}?>"> <!-- has-old -->
            <?if ($price['RATIO_DISCOUNT'] > 0)
            {
                ?>
            ...
            <?}?>
            <?if($price['RATIO_PRICE'] <= 0 && $item['PROPERTIES']['SOLDOUT']['VALUE']){?>
                <div class="product__preorder"><?if (LANGUAGE_ID =='en'){echo 'Sold out';}else{ echo 'Распродано';}?></div>
            <?}?>
            <?if($item['PROPERTIES']['PREDZAKAZ']['VALUE']){?>
                <div class="product__preorder"><?if (LANGUAGE_ID =='en'){echo 'Pre-order';}else{ echo 'Предзаказ';}?></div>
            <?}?>
		</div>
	<a href="<?=$item['DETAIL_PAGE_URL']?>" onclick="return true;" class="product__link" title="<?=$brand?> | <?=$productTitle?>"><?=$brand?> | <?=$productTitle?></a>
</div>
