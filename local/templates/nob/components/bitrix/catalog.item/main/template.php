...
{
	$item = $arResult['ITEM'];
	$brand = $item['PROPERTIES']['BREND']['VALUE'];
	...
	if (LANGUAGE_ID == "en")
    {
		$productTitle = $item['PROPERTIES']['ANGLIYSKOE_NAIMENOVANIE']['VALUE'] ? $item['PROPERTIES']['ANGLIYSKOE_NAIMENOVANIE']['VALUE'] : $item['NAME'];
	} else
    {
	...
	if ($arParams['PRODUCT_DISPLAY_MODE'] === 'N' && $haveOffers)
	{
		...
		foreach ($item['PROPERTIES']['MORE_PHOTO']['VALUE'] as $key => $value) {
			$morePhoto[]['SRC']=CFile::GetPath($value);
		}
	...
	?>
		...
		$templatePath = mb_strtolower($arResult['TYPE'],'UTF-8').'/template.php';
