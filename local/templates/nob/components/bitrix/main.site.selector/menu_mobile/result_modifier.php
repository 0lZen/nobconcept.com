...
foreach($arResult["SITES"] as $key => $arSite)
{
	...
	/*$arResult["SITES"][$key]['LINK'] =
        //(is_array($arSite["DOMAINS"]) ? $arSite["DOMAINS"][0] : $arSite["DOMAINS"]).
        ($arSite['LANG'] == LANGUAGE_ID ? $_SERVER['REQUEST_URI'] : ($arSite['LANG'] != DEFAULT_LANG ? '/'.$suffix . $arSite['LANG'] : '')
