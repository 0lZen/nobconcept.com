...
foreach ($this->basketItems as $row)
{
	...
	if (!empty($result['GRID']['HEADERS']) && is_array($result['GRID']['HEADERS']))
	{
		...
		$rowData['BRAND'] = $row['PROPERTY_BREND_VALUE'];
		$rowData['RAZMER'] = $row['PROPERTY_RAZMER_VALUE'];
		$rowData['TSVET'] = $row['PROPERTY_TSVET_VALUE'];
	}
...
foreach ($rowData['PROPERTIES'] as $key => $value) {
	$totalData[$key] = $value;
}
